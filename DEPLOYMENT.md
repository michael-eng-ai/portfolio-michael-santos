...
- Apply [news_reliability.sql](/Users/michaelsantos/Documents/GitHub/portfolio-michael-santos/supabase/news_reliability.sql) on the PostgreSQL database (the `supabase/` directory holds the canonical `.sql` schema files used by the VM bootstrap)
...
### VM PostgreSQL
PostgreSQL (configured by `DATABASE_URL`, optionally `DATABASE_SSL`) is the only database.
...
### Pointing Vercel at the VM PostgreSQL
Only proceed once the VM database is reachable from Vercel over a safe network path.
1. Confirm the VM PostgreSQL is reachable and serving active news: run `pnpm db:cutover:check`.
...
The application code routes every read/write through PostgreSQL (`lib/database.ts`), so no code change is needed — it is purely an environment-variable switch.
...
- Add `DATABASE_URL` (and `DATABASE_SSL` when the database enforces TLS)
...
- Newsletter subscription returns success with a configured PostgreSQL `DATABASE_URL`
...
| `michael-health-check` | Every 5 min | Ping `/api/health` (keeps the database connection warm) |
