 ...
 */
async function generateDailyArticle(): Promise<Article> {
  const openai = new OpenAI();
  const formattedDate = "10 de março de 2026";

  // Tópicos já cobertos - evitar repetição
  const topicosJaCobertos = [
    "Data Mesh", "Apache Kafka", "Apache Flink",
    "LLMOps", "Agentes de IA Autônomos", "IA Agêntica",
    "Observabilidade de Dados", "Synthetic Data Generation",
    "ETL Inteligente", "Feature Stores", "Model Context Protocol",
    "Quantum Computing", "Streaming em Tempo Real",
    "Engenharia de Dados", "Pipelines Inteligentes"
  ];
...
Sugestões de temas interessantes para 10 de março de 2026:
- Apache Iceberg e o futuro dos formatos de tabela aberta (Open Table Formats)
- DataOps e DevDataOps: a nova fronteira da automação de dados
- Arquitetura Kappa vs Lambda em 2026: qual escolher?
- IA Multimodal e impacto na engenharia de dados
- Governança de IA (AI Governance) e compliance com LGPD
...
- Lakehouse Architecture: unificando Data Lakes e Data Warehouses
- Real-Time Analytics com Apache Pinot ou ClickHouse
- dbt (data build tool) e a transformação da camada de modelagem de dados
- Semantic Layer e métricas unificadas para BI moderno
...
FORMATO DE RESPOSTA (JSON):
{
  ...
  try {
    ...
    // ID 74 (próximo após o último artigo id=73)
    const nextId = 74;
    ...
    const outputPath = `article_2026_03_10.json`;
